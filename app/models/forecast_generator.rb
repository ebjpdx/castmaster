class ForecastGenerator
  ...
  def merge_dependencies!(dep = {})
    dep.each do |field,v|
      ...
      # raise TypeError, "The #{field} dependency needs to have a target table of #{self.dependencies[field].target_table}, not #{v.target_table}." unless v.target_table == self.dependency_runs[field].target_table
    end
  ...
  def forecasts_field_values
    ...
    iv.slice(*ForecastRun.column_names.map {|c| c.to_sym} )
  end


  def find_existing_forecast_run
    # conn.verify!
    forecasts = ForecastRun.where(identification_conditions.merge({date_reaped: nil, status: 'completed'})).order('id desc')
    ...
    if forecasts.empty? or current_dependencies.any? {|id|  id.nil?}
      ...
      if self.parameters[:ephemeral]
        ...
        latest_forecast_id_in_table = ForecastRun.where({target_table: self.target_table}).order('id desc').first.id
      ...
      if forecasts.length > 1 then
        Rails.logger.warn {"The forecast_identifiers for #{name} do not specify a unique forecast id. Unless you have specified the force_refresh option, this is an error.
           ForecastRun ID's returned: #{forecasts.map {|f| f.id}}
  ...
  def run(force_refresh=false, debug=false, log_indent='')
    ...
    Rails.logger.info {"#{log_indent}#{name}--Checking Dependencies:"} unless @dependencies.length == 0
    @dependencies.each do |n,d|

      if d.forecast_run.nil? || force_refresh == :all
        Rails.logger.info {"#{log_indent + '  '}#{n}--No matching forecast; force_refresh = #{force_refresh.to_s}"}
        d.run((if force_refresh == :all then force_refresh end), debug, (log_indent ||'') + '    ')
      else
        Rails.logger.info {"#{log_indent + '  '}#{n}--Using existing forecast: #{d.forecast_id}"}
      end
    end

    if self.forecast_run.nil? || force_refresh
      Rails.logger.info {"#{log_indent}#{name}--Generating new forecast: force_refresh = '#{force_refresh.to_s}', parameters = #{self.parameters.except(:dependencies)} "}
      ...
      begin
        ...
        # self.forecast_run.save
        Rails.logger.info {"#{log_indent}#{name}--Finished forecast run: forecast_id = #{self.forecast_run.id}."}
      ...
      Rails.logger.info {"#{log_indent}#{name}--Using existing forecast: forecast_id = #{self.forecast_id}, force_refresh = '#{force_refresh.to_s}', parameters = #{self.parameters}"}
    end
    forecast_run
  end
